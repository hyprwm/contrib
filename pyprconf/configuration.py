...
def _resolve_commands(command: str, commands: Dict[str, Any], tokens: Dict[str, str]) -> List[str]:
    return list(
        f'/{command} {keyword}'.format_map(tokens)
        for keyword in commands
    )


def _resolve_binds(binds: Dict[str, str], tokens: Dict[str, str]) -> Tuple[List[str], List[str]]:
    resolved = {key.format_map(tokens): cmd.format_map(tokens)
                for (key, cmd) in binds.items()}
    return list(
        f'/keyword bind {key},{cmd}'
        for key, cmd in resolved.items()
    ), list(
...
def _register_handlers(handlers: HandlerDict, events: List[Dict[str, Any]], tokens: Dict[str, str]):
    for event in events:
        try:
            commands = batch([
                resolved
                ...
                for resolved in _resolve_commands(command_type, event[command_type], tokens)
            ])
...
def _parse_range(rangedef: Union[Dict[str, Any], None]) -> List[int]:
    if rangedef:
        if len(rangedef) < 3:
...
def parse(configdict: Dict[str, Any], logger: Logger) -> Tuple[Loaders, Configuration]:
    global log
    log = logger
    ...
    for name, section in configdict.items():
        ...
        range_tokens = _parse_range(rangedef)
        ...
        for range_token in range_tokens:
            ...
            for command_type in COMMAND_TYPES:
                if command_type in section:
                    loaders.append(_resolve_commands(
                        command_type, section[command_type], tokens))
    ...
    return [batch(loader) for loader in loaders], Configuration([batch(unloader) for unloader in unloaders], handlers)
